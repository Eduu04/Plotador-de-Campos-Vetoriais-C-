...
/**
 * @brief Executa o plotador de campos vetoriais.
 *
 * O programa solicita ao usuário as componentes horizontal e vertical de um campo vetorial,
 * avalia essas funções em uma malha bidimensional e exibe o resultado graficamente através de setas renderizadas com SFML.
 *
...
int main() {
    ...
    // Exibe informações iniciais ao usuário.
    ...
    // Recebe as componentes do campo vetorial.
    std::string fx, fy;
    std::cout << "\nDefina fx(x,y) = ";
    ...
    // Cria os interpretadores das expressões matemáticas.
    Parser fx_exprtk(fx);
    Parser fy_exprtk(fy);

    int N = 30;

    //  Configura o armazenamento dos vetores calculados.
    int Total_Pontos = N*N;
    DatasetVetor dataset(Total_Pontos);

    // Cria a janela de renderização e as estruturas gráficas.
    ...
    /**
     * @brief Gera os vetores do campo vetorial e suas representações gráficas.
     *
     * Percorre uma malha bidimensional de N × N pontos distribuídos no domínio de visualização,
     * avalia as componentes horizontal e vertical do campo vetorial em cada posição utilizando as
     * expressões fornecidas pelo usuário, armazena os valores calculados no conjunto de dados, normaliza a direção dos vetores
     * direção para manter a consistência visual da representação gráfica, ajusta dinamicamente o comprimento
     * das setas de acordo com a intensidade do campo e cria os objetos gráficos responsáveis pela renderização do campo vetorial na janela.
     *
     */
    for (int i = 0; i<N; i++){
        for (int j = 0; j<N; j++){
            ...
            double vx = vetorAtual.getX();
            double vy = vetorAtual.getY();
            ...
            Vetor2D<double> posInicial(xTela, yTela);
            Vetor2D<double> dirNormal(vx, -vy);

            Vetor2D<double> posFinal = posInicial + (dirNormal * escalaDinamica);
            Rep_Grafica.push_back(std::make_unique<Arrow>(posInicial.getX(), posInicial.getY(), posFinal.getX(), posFinal.getY()));
    ...
    std::cout << "\nQuantidade de vetores calculado: " << dataset.getCapacidade() << std::endl;
